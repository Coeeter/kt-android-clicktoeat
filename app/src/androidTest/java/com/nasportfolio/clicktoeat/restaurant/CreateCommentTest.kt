...
class CreateCommentTest {
    ...
    @Test
    fun whenCreateReview_shouldUpdateDetailsScreen() {
        runBlocking {
            val review = "Test Review"
            composeRule.onAllNodesWithTag(TestTags.FAVORITE_RESTAURANT_TAG)[0]
                .performClick()
            composeRule.awaitIdle()
            composeRule.onNodeWithTag(TestTags.REVIEW_INPUT)
                .performScrollTo()
                .assertIsDisplayed()
                .performTextInput(review)
            composeRule.onAllNodesWithTag(TestTags.STAR)[3]
                .performClick()
            composeRule.onNodeWithTag(TestTags.CREATE_REVIEW_BTN)
                .performClick()
            composeRule.onNodeWithText(review)
                .assertIsDisplayed()
        }
    }
}